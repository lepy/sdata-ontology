"""Tests for sdata-core ontology and instance data (v0.1.0)."""
...
def min_graph():
    g = Graph()
    g.parse(ROOT / "min-v1.0.0.ttl", format="turtle")
    return g
...
def example_graph():
    g = Graph()
    g.parse(ROOT / "examples" / "specimen_tensiontest_data.ttl", format="turtle")
    return g
...
def test_core_uses_min_bases_via_facades(core_graph):
    assert (SDATA.Object, RDFS.subClassOf, MIN.Object) in core_graph
    assert (SDATA.Agent, RDFS.subClassOf, MIN.Agent) in core_graph
    assert (SDATA.Data, RDFS.subClassOf, MIN.Data) in core_graph
    assert (SDATA.Process, RDFS.subClassOf, MIN.Process) in core_graph
    assert (SDATA.Material, RDFS.subClassOf, SDATA.Object) in core_graph
    assert (SDATA.Person, RDFS.subClassOf, SDATA.Agent) in core_graph
...
def test_core_has_sdata_process_class(core_graph):
    assert (SDATA.Process, RDF.type, OWL_CLASS) in core_graph
